...
public class SecondaryHDLCDataLink {
	 ...
	 *  Connection Service
	 *-----------------------------------------------------------*/
	// This is a confirmed service, i.e. the return value reflects results from the
	// confirmation
	...
	public Result dlConnectIndication() { // Receive NRM command frame
		...
		if (type.equals(HdlcDefs.U_FRAME) == false) {
			...
			if (uframe.equals(HdlcDefs.SNRM) == false) {
				...
				System.out.println("Data Link Layer: received SNRM frame >" + BitString.displayFrame(frame) + "<");
		}
	...
	public Result dlDisconnectIndication() { // Disconnection to secondary.
		...
		if (type.equals(HdlcDefs.U_FRAME) == false) {
			...
			if (uframe.equals(HdlcDefs.DISC) == false) {
				...
				System.out.println("Data Link Layer: received DISC frame >" + BitString.displayFrame(frame) + "<");
		}
		return (new Result(cd, adr, retStr));
	}
	...
	private String getFrame(boolean wait) {
		...
		do {
			if (wait)
				...
				frame = physicalLayer.pollReceive(); // get frame from physical layer
			if (frame != null) {
		...
		// if(frame != null) System.out.println("Data Link Layer: Received frame
